...
tab_diagnose, tab_eval, tab_calendar = st.tabs(
    ["🔍 Diagnose", f"📊 {t('eval_header', lang)}", "📅 Crop Calendar"]
)
...
# ---------------------------------------------------------------------------
# TAB 3 — Crop Calendar (Sindh sowing/harvest windows)
# ---------------------------------------------------------------------------
CROP_CALENDAR = {
    "Wheat": {"season": "Rabi (winter)", "sowing": "October – November", "harvest": "March – April"},
    "Rice": {"season": "Kharif (monsoon)", "sowing": "June – July", "harvest": "October – November"},
    "Cotton": {"season": "Kharif (monsoon)", "sowing": "April – May", "harvest": "September – December"},
    "Mango": {"season": "Perennial (tree crop)", "sowing": "Flowering: January – February", "harvest": "May – August"},
}

with tab_calendar:
    st.subheader("📅 Sindh Crop Calendar")
    st.caption("General sowing/harvest windows for Sindh. Exact dates vary by district and weather.")
    cal_df = pd.DataFrame([
        {"Crop": crop, "Season": info["season"], "Sowing": info["sowing"], "Harvest": info["harvest"]}
    ...
    st.download_button(
        ...
        file_name="sindh_crop_calendar.csv",
        mime="text/csv",
    )

# ---------------------------------------------------------------------------
# Sidebar: local weather (Open-Meteo — free, no API key)
# ---------------------------------------------------------------------------
import requests
...
with st.sidebar:
    ...
    weather = get_weather(*SINDH_LOCATIONS[city])
    if weather:
