...
def get_sample_images():
    ...
    for cls in CLASS_NAMES:
        files = glob.glob(
            os.path.join(SAMPLE_DIR, cls, "*.png")
        )
...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        "Model",
        options=["mobilenet", "baseline_cnn"],
    ...
    st.caption(
        "Compare both models in the Evaluation tab below."
    )
    ...
    city = st.selectbox(
        "Your area",
        list(SINDH_LOCATIONS.keys())
    )

    latitude, longitude = SINDH_LOCATIONS[city]
...
tab_diagnose, tab_eval = st.tabs(
    [
        "🔍 Diagnose",
        f"📊 {t('eval_header', lang)}"
    ]
)
...
with tab_diagnose:
    ...
    col_input, col_samples = st.columns(
        [2, 1]
    )
...
with tab_eval:
    ...
    if metrics is None:
        ...
        st.warning(
            ...
            "Run `python train.py` to generate evaluation results."
        )

    else:

        st.subheader(
            "Baseline CNN vs. MobileNetV2 (transfer learning)"
        )
        ...
        st.caption(
            ...
            "saved_models/metrics.json for further inspection."
        )
